...
export function removeUnit(s: string): number {
    if(s.endsWith("px") || s.endsWith("em")) return parseFloat(s.substring(0, s.length-2))

    return parseFloat(s);
}

export function getLetterWidth(canvas: HTMLCanvasElement, font_size: number): number {
